...
const Station = () => {
  ...
  const handleBeginStation = async () => {
    ...
    try {
     ...
     vapi.on("call-end", () => {
      ...
      const call = await vapi.start(caseData.vapi_assistant_id);
  ...
  const pollForReport = (sid: string | null) => {
    if (!sid) return;
    ...
    const poll = setInterval(async () => {
      ...
      if (data?.status === "completed") {
        ...
        setGenerating(false);
        setErrorMessage(
          "Feedback generation failed. Please contact support."
        );
        setStep("error");
        ...
        setGenerating(false);
        setErrorMessage(
          "Feedback is taking longer than expected. Check your session history in a few minutes."
        );
        setStep("error");
      }
    }, 3000);
          ...
          <div className="space-y-8">
              ...
              <CardContent className="p-6 sm:p-8">
                <div
                  ...
                  dangerouslySetInnerHTML={{
                    __html: caseData.brief_markdown
                      .replace(
                        /^## (.+)$/gm,
                        '<h2 class="text-sm font-semibold uppercase tracking-wider text-muted-foreground mt-4 mb-2">$1</h2>'
                      )
                      .replace(
                        /^### (.+)$/gm,
                        '<h3 class="text-xs font-semibold uppercase tracking-wider text-muted-foreground mt-4 mb-2">$1</h3>'
                      )
                      .replace(
                        /^\d+\. (.+)$/gm,
            ...
            <div className="text-center">
              <p className="mb-4 text-muted-foreground">
                You have{" "}
                <strong>
                  {Math.floor(caseData.duration_seconds / 60)} minutes
                </strong>
              </p>
              <Button
                size="lg"
